...
public class TopicRoomChatPushFlushScheduler {
    ...
    public void flush() {
        List<Long> dueRoomIds = redisTopicRoomPushBatchAdapter.pollDueRoomIds(FLUSH_LIMIT);
        if (dueRoomIds.isEmpty()) return;

        for (Long roomId : dueRoomIds) {
            try {
                if (!redisTopicRoomPushBatchAdapter.tryAcquireSendSlot(roomId)) continue;

                // 발송이 1초 틱을 잡지 않도록 방 단위로 떼어낸다. 게이트와 회수는 여기서 끝내둔다
                topicRoomChatPushFlusher.flush(roomId, redisTopicRoomPushBatchAdapter.drain(roomId));
            } catch (Exception e) {
    ...
    public void recoverMissedRooms() {
        ...
        for (Map.Entry<Long, Long> entry : lastMessageIds.entrySet()) {
            Long anchor = redisTopicRoomPushBatchAdapter.findLastPushedMessageId(entry.getKey());

            // 앵커가 없으면 이미 보냈는지 판단할 수 없어 건드리지 않는다
            if (anchor == null || entry.getValue() <= anchor) continue;

            // 이미 대기열에 있으면 묶음 창에서 정상 대기 중인 것이라 복구가 아니다
            if (redisTopicRoomPushBatchAdapter.enqueue(entry.getKey())) recovered++;
        }
